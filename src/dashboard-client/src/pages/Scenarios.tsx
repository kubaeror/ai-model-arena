...
export function Scenarios() {
  ...
  if (mode.kind === 'edit') {
    return (
      <div className="p-6">
        {editQuery.isLoading ? (
          <div className="flex items-center gap-2 text-fg-1 text-sm"><Spinner /> Loading…</div>
        ) : editQuery.data ? (
          <ScenarioForm
          ...
          <div className="text-fg-1 text-sm">Scenario not found.</div>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-1">
      ...
      <Panel className="divide-y divide-border">
        {list.isLoading ? (
          <div className="p-1 flex items-center gap-2 text-fg-1 text-sm"><Spinner /> Loading…</div>
        ) : list.data && list.data.length ? (
          list.data.map((s: ScenarioConfig) => (
            <div key={s.name} className="flex items-center justify-between p-3">
              <div>
                <div className="font-medium text-sm">{s.name}</div>
                <div className="text-xs text-fg-1">{s.description ?? s.task.slice(0, 80)}</div>
                <div className="text-xs text-fg-1 mt-0.5">
          ...
          <div className="p-6 text-fg-1 text-sm text-center">No scenarios yet. Click “New scenario” to create one.</div>
        )}
      </Panel>
    </div>
  );
