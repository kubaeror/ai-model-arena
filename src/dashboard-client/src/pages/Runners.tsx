        ...
        <PanelBody>
          {isLoading ? <Spinner /> :
           !runners || runners.length === 0 ? <EmptyState title="No runners deployed" /> : (
            <div className="flex flex-col gap-16">
              {runners.map((r) => (
                <div key={r.name} className="border border-border rounded-lg p-12">
                  ...
                  {r.pods.length > 0 && (
                    <table className="w-full font-mono text-12">
                      <thead><tr className="text-fg-1 uppercase border-b border-border">
                        <th className="px-4 py-4 text-left">Pod</th>
                        <th className="px-4 py-4 text-left">Status</th>
                        <th className="px-4 py-4 text-left">Node</th>
                      </tr></thead>
                      <tbody>
                        {r.pods.map((p) => (
                          <tr key={p.name} className="border-b border-border/50">
                            <td className="px-4 py-4">{p.name}</td>
                            <td className="px-4 py-4"><Badge variant="status" value={p.status} /></td>
                            <td className="px-4 py-4 text-fg-1">{p.node}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
